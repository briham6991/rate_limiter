...
class CustomUser(AbstractUser):
    ...
    company_id = models.ForeignKey(CompanyInformation, on_delete=models.PROTECT, null=True, blank=True, db_column='company_id')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def clean(self):
        pass # TODO: Implement the validation that whenever superadmin is created company_id should be null and whenever admin or readonly user is created company_id should not be null and also check for the role value and other things that are important to maintain the integrity of the data in the database. This clean method will be called before saving the user instance to the database and if any validation error occurs it will raise a ValidationError which can be handled in the views and appropriate response can be sent to the client.
...
class PlanInformation(models.Model): 
    ...
    requests_per_month = models.BigIntegerField(default=100000)
